...
async def generate_video(request: VideoGenerationRequest):
    ...
    try:
        ...
        # Include Creatomate URL in the response
        if response.creatomate_video_url is not None:
            logger.info(f"Creatomate video URL included in response: {response.creatomate_video_url}")
        else:
            logger.warning("Creatomate video URL is not available in the response")
        
        logger.info("Creatomate URL included in response as requested")
